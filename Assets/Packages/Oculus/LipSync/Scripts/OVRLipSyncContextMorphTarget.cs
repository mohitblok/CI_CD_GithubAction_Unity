...
public class OVRLipSyncContextMorphTarget : MonoBehaviour
{
    ...
    private float morphTargetValue;
    ...
    void Start ()
    {
        ...
        morphTargetValue = skinnedMeshRenderer.name == "Wolf3D.Avatar_Renderer_Teeth" ? 200f : 150f;
        ...
        for (int i = 0; i < visemeToBlendTargets.Length; i++)
        {
            if (visemeToBlendTargets[i] != -1)
            {
                ...
                skinnedMeshRenderer.SetBlendShapeWeight(
                    ...
                    frame.Visemes[i] * morphTargetValue);
            }
        }
    }
    ...
    void CheckVisemeKey(KeyCode key, int viseme, int amount)
    {
        ...
        {
            lipsyncContext.SetVisemeBlend(visemeToBlendTargets[viseme + 5], amount);
        }
        if (Input.GetKeyUp(key))
        {
            lipsyncContext.SetVisemeBlend(visemeToBlendTargets[viseme + 5], 0);
        }
    }
